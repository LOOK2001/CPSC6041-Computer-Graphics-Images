...
void ImageOperator::okwarp(Image* in, Image* out, int mode)
{
    int in_width = in->Width();
    int in_height = in->Height();
    ...
    for (int i = 0; i < out_width; i++)
    {
        for (int j = 0; j < out_height; j++)
        {
            for (int k = 0; k < nChannels; k ++)
            {
                float u, v;
                if (mode == 0)
                    inv_map(i, j, u, v, in_width, in_height, out_width, out_height);
                else
                    inv_map2(i, j, u, v, in_width, in_height, out_width, out_height);
...
void ImageOperator::antialising (Image* in, Image* out, int mode)
{
    int in_width = in->Width();
    int in_height = in->Height();
            ...
            for (int k = 0; k < nChannels; k ++){
                ...
                for (int i = -size; i <= size; i++){
                    for (int j = -size; j <= size; j ++){
                        if (u+i>=0 && u+i<in_width && v+j>=0 && v+j<in_height){
                            num += filter[(i+1)*3+(j+1)];
                            value += in->value(u+i, v+j, k) * filter[(i+1)*3+(j+1)];
                        }
                    }
                }
                value /= num;
                out->value(i, j, k) = value;

                // Adaptive
                if (mode == 1){
                    // calculate the are average
                    int num = 0;
                    int size = 2;
                    float average = 0.0;
                    ...
                    for (int i = -size; i <= size; i+=2){
                        for (int j = -size; j <= size; j+=2){
                            if (u+i>=0 && u+i<in_width && v+j>=0 && v+j<in_height){
                                num ++;
                                average += in->value(u+i, v+j, k);
                            }
                        }
                    }
                    average /= num;

                    // If the average value larger than threshold, compute the around pixels within smaller size area
                    if ((in->value(i, j, k) - average) > threshold){
                        num = 0;
                        size = 1;
                        for (int i = -size; i <= size; i++){
                            for (int j = -size; j <= size; j ++){
